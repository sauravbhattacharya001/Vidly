...
class BehavioralFingerprinter:
    ...
    def _timing_similarity(self, a: TimingProfile, b: TimingProfile) -> float:
        ...
        # If both vectors are zero (identical no-activity profiles), they're identical
        if all(x == 0.0 for x in va) and all(x == 0.0 for x in vb):
            return 1.0
    ...
    def _resource_similarity(
        ...
        # If both vectors are zero (identical no-usage profiles), they're identical
        if all(x == 0.0 for x in va) and all(x == 0.0 for x in vb):
            return 1.0
        # Normalize
        for i in range(len(va)):
